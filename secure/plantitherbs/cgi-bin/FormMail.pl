...
BEGIN
{
  ...
  $postmaster        = '';
  @referers          = qw(plantitherbs.com elinuxservers.com localhost);
  @allow_mail_to     = qw(plantitherbs.com frognet.net);
  @recipients        = ();
  %recipient_alias   = ();
...
BEGIN {
  $script = __PACKAGE__->new(
     DEBUGGING              => $DEBUGGING,
     name_and_version       => 'NMS FormMail 3.11c1',
