...
sub close
{
	...
	$self->_dump_packet($quit_message) if DBD::MySQL->debug;
	$mysql->close;
}
...
sub create_record_iterator
{
	...
	my $record = DBD::MySQL::RecordIterator->new(
...
sub _request_authentication
{
	...
	$self->_dump_packet($auth_result) if DBD::MySQL->debug;
...
sub _execute_command
{
	...
	$self->_dump_packet($message) if DBD::MySQL->debug;

	my $result;
	$mysql->recv($result, BUFFER_LENGTH, 0);
	$self->_dump_packet($result) if DBD::MySQL->debug;
	$self->_reset_status;
...
sub _get_record_by_server
{
	...
	while ($self->_has_next_packet($packet)) {
		...
		$packet .= $next_result;
		$self->_dump_packet($next_result) if DBD::MySQL->debug;
	}
...
package DBD::MySQL::RecordIterator;
use strict;
...
DBD::MySQL - Pure Perl MySQL network protocol interface.

=head1 SYNOPSIS

  use DBD::MySQL;
  
  my $mysql = DBD::MySQL->new(
...
There are two classes which have public APIs: DBD::MySQL and DBD::MySQL::RecordIterator.
...
This DBD::MySQL::RecordIterator object can be used for multiple purposes. First of all you can retreive a row of data:
...
The constructor of DBD::MySQL. Connection with MySQL daemon is established and the object is returned. Argument hash contains following parameters:
...
When SELECT type SQL is specified, DBD::MySQL::RecordIterator object which shows the reference result is returned.

    $mysql->query(q{SELECT * FROM table});
    my $a_record_iterator = $mysql->create_recrod_iterator();

DBD::MySQL::RecordIterator object is applicable to acquisition of a reference result. See L<"/Net::SQL::RecordIterator API"> for more.
...
=head2 DBD::MySQL::RecordIterator API

DBD::MySQL::RecordIterator object is generated by the query() method of DBD::MySQL object. Thus it has no public constructor method.
...
This list is the environment which I can use by the test usually. DBD::MySQL will operate  also in much environment which is not in a list.
